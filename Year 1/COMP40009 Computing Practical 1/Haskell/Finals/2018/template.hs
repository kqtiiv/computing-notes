...
-- update (id, x) s = (id, x) : [ (id', x') | (id', x') <- s, id/=id' ]
update (id, x) s = (id, x) : filter ((/= id).fst) s
...
execStatement (Assign id e) acc = update (id, eval e acc) acc 
execStatement (If e b1 b2) acc = case eval e acc of 
  1 -> execBlock b1 acc 
  0 -> execBlock b2 acc 
execStatement stmt@(DoWhile blk e) acc = case eval e acc' of 
  1 -> execStatement stmt acc'
  0 -> acc'
  where acc' = execBlock blk acc

-- execBlock :: [Statement] -> State -> State
execBlock :: Block -> State -> State
execBlock blk s = foldl' (flip execStatement) s blk 
...
foldConst (Phi (Const c) (Const c'))
  | c == c' = Const c 
foldConst (Apply op (Const c) (Const c')) = Const $ apply op c c' 
...
sub id x (Var id') 
  | id == id' = Const x
sub id x (Apply op e1 e2) = 
  foldConst (Apply op (sub id x e1) (sub id x e2))
sub id x (Phi e1 e2) = 
  foldConst (Phi (sub id x e1) (sub id x e2))
sub _ _ e = e
...
scan :: Block -> Id -> Int -> (Block, Worklist)
-- scan :: (Exp -> Exp) -> Block -> (Exp -> Exp, Block)
scan (stmt:blk) v c = case stmt of 
  Assign v' e -> case (sub v c e) of 
    (Const c') -> 
      let (blk', wl) = scan blk v c
      in if v'=="$return" 
        then (stmt:blk', wl) 
        else (blk', (v', c'):wl)
    e' -> 
      let (blk', wl) = scan blk v c
      in (Assign v' e':blk', wl)
  If e blk' blk'' -> 
    let (modBlk, wl) = scan blk' v c 
        (modBlk', wl') = scan blk'' v c 
        (modBlk'', wl'') = scan blk v c 
    in ((If (sub v c e) modBlk modBlk'):modBlk'', wl ++ wl' ++ wl'')
  DoWhile blk' e -> 
    let (modBlk, wl) = scan blk' v c 
        (modBlk', wl') = scan blk v c 
    in ((DoWhile modBlk (sub v c e)):modBlk', wl++wl')
scan [] _ _ = ([], [])

propagateConstants :: Block -> Block
-- Pre: the block is in SSA form
propagateConstants blk = helper wl blk'
  where 
    (blk', wl) = scan blk "$INVALID" 0

helper :: Worklist -> Block -> Block 
helper [] blk = blk 
helper ((v, c):wl) blk = helper (wl ++ wl') blk'
  where (blk', wl') = scan blk v c 
...
unPhi :: Block -> Block
-- Pre: the block is in SSA form
unPhi ((If e blk' blk''):blk) =
  let (t, f, rem) = processBlk blk
  in ((If e ((unPhi blk')++t) ((unPhi blk'')++f)):unPhi rem)
unPhi ((DoWhile blk' e):blk) = 
  let (t, f, rem) = processBlk
  in ((DoWhile (t++rem++f) e):(unPhi blk))
unPhi blk = blk

processBlk :: Block -> (Block, Block, Block)
processBlk blk = (pre, post, rem)
  where 
    (phis, rem) = span isPhi blk 
    (pre, post) = unzip $ map processPhi phis 

processPhi :: Statement -> (Statement, Statement)
processPhi (Assign x (Phi e e')) = 
  (Assign x e, Assign x e')


isPhi :: Statement -> Bool 
isPhi (Assign x (Phi e e')) = True 
isPhi _ = False
