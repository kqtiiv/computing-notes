...
{
	bst binst = malloc(sizeof(struct bst));

	if (binst == NULL) { return NULL; }
...
static bintree makenode( char *key, void *value )
{
	...
	bintree new_node = malloc(sizeof(struct bintree));
...
void add_bst( bst b, char *key, void *value )
{
	...
	bintree new_node = makenode(key, value);
	assert(binst != NULL);
